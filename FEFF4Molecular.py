...
class FEFF4Molecular(coords_file):
	...
	def newfolder_runFEFF(self):
		...
		os.system('mv ' + self.path + '/' + coords_file + ' '+self.path + '/' + coords_file[:-4] + '/')
		os.system('mv ' + self.path + '/autorun.sh' + ' ' + self.path + '/' + coords_file[:-4] + '/')
		os.chdir(self.path+'/'+coords_file[:-4]+'/')
		print('check current directory in subfolder: ',self.path+'/'+coords_file[:-4]+'/')
	...
	def average(self,file_list, data_cols_to_average):
	    ...
	    '''
	    # first check that the data are all of the same size (nrows)
	    print('Averaging:')
	    print(file_list)
	    ...
	    for ifile in file_list:
	        ...
	        for icol in data_cols_to_average:  # data_cols_to_average need to be in sequence
	        ...
	        if run_once == 0:
	            ...
	            # print(data_in)
	            run_once +=1
	        print('got this file data recorded  ',ifile)
    ...
    def average_allcols(self,group):  # note group is the filelist actually
	    ...
	    if (ncols == 6):
	        avg = average(group, np.arange(3,6))
	        print('average calculation successful')
	    else:
	        print('column number not right,change code or change file')

	    if (self.out_dir == None):
	        file_out = os.path.basename('{:s}simulation_Cd{:s}{:d}_{:s}{:s}.dat'.format(side, sec_atom, cd_num, atom_edge, KL23_edge))  # save in present directory
	    else:
	        file_out = self.out_dir + '/' + os.path.basename('EXAFSsimulation_Cd{:s}{:d}_{:s}{:s}.dat'.format(sec_atom, cd_num, atom_edge, KL23_edge))
	...
	self.average_allcols(flie_list)
